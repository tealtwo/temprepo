...
import me.whitehatd.aquila.queue.menu.panel.stats.match.InventoryDisplayMenu;
...
public class ReportPlayerStatsMenu extends GlobalMenu {
    ...
    public ReportPlayerStatsMenu(Match match, UUID targetUuid) {
        super(4, Text.translate("&5Player Match Stats"), InteractionModifier.VALUES);
    ...
    private void populateStats() {
        ...
        if (stats == null) {
            ...
            getDecorator().add('p', ItemBuilder.from(Material.BARRIER)
                    ...
                    .lore(List.of(Text.translate("&7No statistics for this player")))
                    .build());
            // Back
            getDecorator().add('r', ItemBuilder.from(Material.ARROW)
                    .name("&cBack")
                    .menuItem(event -> {
                        new ReportMatchPlayersMenu(match).open(event.getWhoClicked());
                    }));
            return;
        }
        ...
        getDecorator().add('i', ItemBuilder.from(Material.CHEST)
                ...
                .menuItem(event -> {
                    new ReportInventoryMenu(match, targetUuid).open(event.getWhoClicked());
                }));
